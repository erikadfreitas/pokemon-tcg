...
export class EditComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      this.deckKey = params['deckKey'];
      this.deck = this.storeService.getDeckByKey(params['deckKey'])
    });

    this.validDeckKey = this.storeService.checkIfDeckExistsByKey(this.deckKey);

    if (this.validDeckKey) {
      this.deckName = this.deck.deckName;
      this.selectedCards = this.deck.deckCards;
      this.loadSupertypes();
    }
  }
  ...
  removeFromSelectedCards(card: any): void {
    ...
    if (this.lastSearchTermValue && card.name.toLowerCase().includes(this.lastSearchTermValue.toLowerCase())) {
  ...
  submit(): void {
    ...
    this.storeService.createOrUpdateDeck(data, this.deckKey);
